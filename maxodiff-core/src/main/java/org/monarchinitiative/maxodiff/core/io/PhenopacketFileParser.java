...
public class PhenopacketFileParser {
    ...
    public static PhenopacketData readPhenopacketData(Path phenopacketPath) throws MaxodiffDataException, PhenopacketImportException {
        ...
        if (data == null) {
            try (InputStream is = new BufferedInputStream(Files.newInputStream(phenopacketPath))) {
                ...
                throw new MaxodiffDataException("Unable to parse phenopacket from " + phenopacketPath.toAbsolutePath());
            }
        }

        // Check we have exactly one disease ID.
        if (data.diseaseIds().isEmpty())
            throw new MaxodiffDataException("Missing disease ID which is required for the benchmark!");
        else if (data.diseaseIds().size() > 1)
            throw new MaxodiffDataException("Saw >1 disease IDs {}, but we need exactly one for the benchmark!");
        return data;
    }
