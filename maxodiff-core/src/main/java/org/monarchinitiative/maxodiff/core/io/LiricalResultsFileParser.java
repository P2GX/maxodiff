...
public class LiricalResultsFileParser {
    ...
    public static List<LiricalResultsFileRecord> read(InputStream is) throws IOException {
        List<LiricalResultsFileRecord> builder = new ArrayList<>();
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(is));
            ...
            for (CSVRecord record : parser) {
                if (NumberUtils.isParsable(record.get(0))) {
                    ...
                    LiricalResultsFileRecord liricalRecord = new LiricalResultsFileRecord(omimId, omimLabel, posttestProb, LR);
                    builder.add(liricalRecord);
                }
            }
        }
    ...
    public static List<LiricalResultsFileRecord> read(Path path) throws IOException {
